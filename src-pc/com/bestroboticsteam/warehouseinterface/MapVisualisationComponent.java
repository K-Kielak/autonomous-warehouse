...
public class MapVisualisationComponent extends JComponent {
	...
	public MapVisualisationComponent(LineMap _lineMap, float _scaleFactor) {
		...
		m_visualisationDimensions = new Rectangle(scale(_width) + (2 * X_MARGIN), scale(_height) + (2 * Y_MARGIN));
	...
	private Line[] staticTransformMapLines(LineMap _lm, int _dx, int _dy) {
		...
		for (int i = 0; i < originalLines.length; i++) {
			translatedLines[i] = new Line(scale(originalLines[i].x1) + _dx, scale(flipY(originalLines[i].y1)) + _dy,
					scale(originalLines[i].x2) + _dx, scale(flipY(originalLines[i].y2)) + _dy);
		}
	...
	protected void renderMap(Graphics2D g2) {
		if (m_lineMap != null) {
			...
			g2.drawString("(0,0)    0 heading ->", scale(0), scale(flipY(0)) + Y_MARGIN + 15);
			g2.drawString("(0," + mapHeight + ")", scale(0), scale(flipY(mapHeight)) + Y_MARGIN - 5);
			float mapWidth = m_lineMap.getBoundingRect().width;
			g2.drawString("(" + mapWidth + "," + mapHeight + ")", scale(mapWidth) + X_MARGIN,
	...
	private void renderRobot(Graphics2D g2, MobileRobot _robot) {
		...
		renderRelative(_robot.getFootprint(), _robot.getPose(), g2);
		if (m_trackRobots) {
	...
	protected void renderLine(Point _p1, Point _p2, Graphics2D _g2) {
		Line2D line = new Line2D.Double(scale(_p1.x) + X_MARGIN, scale(flipY(_p1.y)) + Y_MARGIN,
				scale(_p2.x) + X_MARGIN, scale(flipY(_p2.y)) + Y_MARGIN);
		_g2.draw(line);
	}
	...
	protected void renderPose(Pose _pose, Graphics2D _g2) {
		Ellipse2D ell =
				...
				new Ellipse2D.Float(scale(_pose.getX() - ROBOT_RADIUS) + X_MARGIN,
						scale(flipY(_pose.getY() + ROBOT_RADIUS)) + Y_MARGIN, scale(ROBOT_RADIUS * 2),
						scale(ROBOT_RADIUS * 2));
		_g2.draw(ell);

		drawLineToHeading(_g2, _pose.getX(), _pose.getY(), _pose.getHeading(), ROBOT_RADIUS * 2);
	}
	 ...
	 * @return
	 */
	public static MapVisualisationComponent createFromSimulation(MapBasedSimulation _sim) {
		MapVisualisationComponent visualisation = new MapVisualisationComponent(_sim.getMap(), 75f);
	 ...
	 * @return
	 */
	public static void populateVisualisation(MapVisualisationComponent _viz, MapBasedSimulation _sim) {
